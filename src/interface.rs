use super::cli::cli::CLI;
use super::password_manager::password_manager::{Password, PasswordManager};
use std::fs;
use std::fs::File;
use std::io::Write;
...
impl PasswordManagerInterface {
    ...
    fn print_password(&self, password: &Password, key: &Option<String>, index: usize) {
        ...
        if password.encrypted {
            println!(
                ...
                match self
                    .pw_core
                    .decrypt(&password.password, key.clone().unwrap().as_str())
                {
                    Ok(val) => val,
                    Err(_) => {
    ...
    pub fn save_password(
        ...
        if encrypt {
            ...
            self.pw_core
                .save_password(&generated_password, &uname, &place, encrypt, None);
        }

        println!(
            "\nSaved Password:\n  password = {}\n  username = {}\n  place = {}",
            generated_password, uname, place
        );
    }
    ...
    pub fn delete_password(&self, place: String) {
        ...
        if password.len() == 0 {
            ...
            if self
                .cli
                .prompt("Are you sure you want to delete this password? [y/n]:")
                .unwrap()
                .as_str()
                == "y"
            {
    ...
    pub fn add_password(&self, password: &str, username: &str, place: &str, encrypt: bool) {
        if encrypt {
            self.pw_core
                .save_password(password, username, place, encrypt, Some(&self.get_key()));
        } else {
            self.pw_core
                .save_password(password, username, place, encrypt, None);
        }

        println!(
            "\nSaved Password:\n  password = {}\n  username = {}\n  place = {}",
            password, username, place
        );
    }
    ...
    pub fn create_backup(&self, path: PathBuf, encrypt: bool) {
        ...
        let mut file = match File::create(path.join(if encrypt {
            "password_backup.txt"
        } else {
            "password_backup.csv"
        })) {
        ...
        if encrypt {
            let file_key = self
                .cli
                .prompt_loop_password("File key (used to encrypt and later decrypt file): ");
    ...
    pub fn restore_backup(&self, file: PathBuf, encrypt: bool) {
        ...
        if !file_contents.contains(",") {
            processing_string = match self
                .pw_core
                .decrypt(&file_contents, &self.cli.prompt_loop_password("File key: "))
            {
